...
// function added by iosk
// Same with l1partition, only that the buckets estimation is done without adding any noise.
// the purpose is to see that given a histogram x, a budget/ratio e*ratio to find what is the optimal partitioning
void L1partition_true(int *hist, int n1, int* x, int n, double epsilon, double ratio, long seed)    {
    double **score = new double*[n];
    double invepsilon2 = 1.0 / (epsilon - epsilon*ratio);
    MTRand53 rnum;
    rnum.seed(seed);
